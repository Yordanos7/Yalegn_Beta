...
export const Component = () => {
  ...
  const gradientFrom = theme === "dark" ? "from-black" : "from-white";
  const gradientVia = theme === "dark" ? "via-black/90" : "via-white/90";
  const gradientTo = theme === "dark" ? "to-black" : "to-white";
  const gradientToTransparent =
    theme === "dark" ? "to-transparent" : "to-transparent"; // This can remain the same
  ...
  return (
    <>
      ...
      >
        {/* Background gradient */}
        <div
          className={`absolute inset-0 bg-gradient-to-b ${gradientFrom} ${gradientVia} ${gradientTo} z-0`}
        />
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t ${gradientFrom} ${gradientToTransparent} z-20`}
        />
      </div>
    </>
  );
