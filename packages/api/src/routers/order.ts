...
export const orderRouter = router({
  ...
  getOrdersForBuyer: protectedProcedure.query(async ({ ctx }) => {
    const { userId } = ctx.session;
      ...
      if (buyer) {
        await prisma.notification.create({
          data: {
            userId: buyer.id,
            type: "SYSTEM", // Or a more specific type like "ORDER_UPDATE"
            title: "Your Order Has Been Shipped! Delivery Proof Attached.",
            body: `Your order ${input.orderId} has been shipped by the seller. Delivery proof (e.g., post office schedule) has been uploaded: ${input.deliveryProofUrl}. Please confirm delivery once you receive it.`,
            payload: {
              orderId: input.orderId,
              sellerId: userId,
              deliveryProofUrl: input.deliveryProofUrl,
  ...
  sellerConfirmProductSent: protectedProcedure
    .input(
      z.object({
        orderId: z.string(),
      })
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const order = await prisma.order.findUnique({
        where: { id: input.orderId },
        select: { sellerId: true, orderStatus: true, buyerId: true },
      });
      ...
      if (order.sellerId !== userId) {
        throw new Error(
          "Unauthorized: Only the seller can confirm product sent."
        );
      }

      if (order.orderStatus !== OrderStatus.PENDING_PAYMENT) {
        throw new Error(
          "Product can only be marked as sent for orders with 'PENDING_PAYMENT' status."
        );
      }
      ...
      // Notify admin about the product sent confirmation
      const adminUsers = await prisma.user.findMany({
        where: { role: "ADMIN" },
        select: { id: true },
      });
      ...
      for (const admin of adminUsers) {
        await prisma.notification.create({
          data: {
            userId: admin.id,
            type: "SYSTEM",
            title: "Product Sent by Seller",
            body: `Seller ${userId} confirmed product sent for order ${input.orderId}. Status updated to DELIVERY_PENDING.`,
            payload: {
              orderId: input.orderId,
              sellerId: userId,
            },
      ...
      // Notify the buyer that the product has been sent
      const buyer = await prisma.user.findUnique({
        where: { id: order.buyerId },
        select: { id: true },
      });
      ...
      if (buyer) {
        await prisma.notification.create({
          data: {
            userId: buyer.id,
            type: "SYSTEM",
            title: "Your Product Has Been Sent!",
            body: `Your order ${input.orderId} has been sent by the seller. Please confirm delivery once you receive it.`,
            payload: {
              orderId: input.orderId,
              sellerId: userId,
            },
  ...
  confirmDeliveryByBuyer: protectedProcedure
    .input(
      z.object({
        orderId: z.string(),
      })
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const order = await prisma.order.findUnique({
        where: { id: input.orderId },
        select: { buyerId: true, orderStatus: true, sellerId: true },
      });
      ...
      if (order.buyerId !== userId) {
        throw new Error(
          "Unauthorized: Only the buyer can confirm delivery for this order."
        );
      }

      if (order.orderStatus !== OrderStatus.DELIVERED) {
        throw new Error(
          "Delivery can only be confirmed for orders with 'DELIVERED' status."
        );
      }
      ...
      for (const admin of adminUsers) {
        await prisma.notification.create({
          data: {
            ...
            body: `Buyer ${userId} confirmed delivery for order ${input.orderId}. Status updated to COMPLETED.`,
            payload: {
              orderId: input.orderId,
              buyerId: userId,
            },
